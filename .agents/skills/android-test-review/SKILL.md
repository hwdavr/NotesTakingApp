...
- `docs/current/test_review_v<N>.md` (Testing stage output — source of truth for test review results)

---

## Execute

### B1. Read the test review
Open `docs/current/test_review_v<N>.md`. Do not re-run `testDebugUnitTest` or `koverLog` — these results are already recorded by the Testing stage.

### B2. Coverage distribution review
From the review's coverage numbers, assess:
...
For each test file listed in the review, review the source:
...
1. Confirm from the review that the regression test was listed as failing before the fix.
...
Produce:
- `docs/current/test_review_v<N>.md` by copying and filling in the template from `docs/templates/test-review-template.md`.

Update `docs/current/summary_v<N>.md`: mark the Review stage complete with overall verdict (combining code and test review verdicts).
...
- [ ] Test review confirms: `testDebugUnitTest` exit code 0
- [ ] Test review confirms: overall ≥ 80%, new classes ≥ 90%
...
- [ ] `docs/current/test_review_v<N>.md` exists with all sections completed and overall verdict filled in
