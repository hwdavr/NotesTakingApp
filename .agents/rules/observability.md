...
All log calls must use `android.util.Log` with a consistent tag:

```kotlin
private const val TAG = "NotesApp/<ClassName>"
Log.d(TAG, "message")
```

- Use the `NotesApp/` prefix on every tag — this is mandatory for logcat filtering.
- Never log PII (email, user ID, note content) at any level.
...
**API / Network**
- Tag: `NotesApp/ApiClient` or OkHttp interceptor output
...
**Room / Database**
- Tag: `NotesApp/Dao` or Room query logs (enable via `RoomDatabase.Builder.setQueryCallback`)
...
```bash
# All app logs
adb logcat -s "NotesApp/*"

# Errors only
adb logcat "*:E" -s "NotesApp/*"

# API traffic only
adb logcat -s "NotesApp/ApiClient" "OkHttp"
```
