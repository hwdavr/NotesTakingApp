...
- When `feature_list.json` declares a visual-verification owner, validate visual traceability with `bash scripts/check-visual-evidence-contract.sh "$FEATURE_DIR"`; a visual method without a sprint-contract row or successful connected evidence is a review failure.
