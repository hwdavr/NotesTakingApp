...
| TC-US-1-02 | AC-US-1-02 | JVM state/repository integration + instrumented service | app/src/test/java/com/example/notesapp/voice/VoiceRecordingSessionStateReducerTest.kt#activeSessionPausesAndResumesFromSameElapsedValue; app/src/androidTest/java/com/example/notesapp/voice/VoiceRecordingServiceIntegrationTest.kt#foregroundServiceOwnsOnePrivateContiguousFile | Drive active/paused/focus-loss/IO-error/stop/discard events with fake clock and service smoke; cover notification action intents in the service test | Assert legal transitions, paused elapsed value, auto-pause/resume, partial-save copy, notification actions, file cleanup, and no orphan session after discard | ./gradlew testDebugUnitTest --tests "com.example.notesapp.voice.VoiceRecordingSessionStateReducerTest" |
| TC-US-1-03 | AC-US-1-03 | JVM unit + instrumented permission/storage UI | app/src/test/java/com/example/notesapp/voice/RecordingStoragePreflighterTest.kt#blocksRecordingBelowThreshold | Use StatFs fixtures for 50 MB/127 MB/128 MB, no-microphone fixture, and permanent-denial permission fixture | Assert exact error copy, Settings deep-link intent for permanent denial, and foreground service is not started on failed preflight | ./gradlew testDebugUnitTest --tests "com.example.notesapp.voice.RecordingStoragePreflighterTest" |
