...
| TC-US-2-VIS-02 | AC-024 | Visual verification | `app/src/androidTest/java/com/example/notesapp/navigation/WebBookmarkAppShellVisualFlowTest.kt#capturesAddBookmarkPageFromAppShell` | N/A — no API | Capture scope: app-shell; production root: `AppNavHost`. Enter Add Web Bookmark through the signed-in production root, wait for idle, assert the root bottom bar is absent, and capture the active window. | Pull non-empty `visual_evidence/web_bookmark_add_page.png` and compare the real full-page shell, URL-only form, and bottom-pinned actions to the approved mockup; Home/Folders/Settings must not be present. | env ANDROID_SERIAL=emulator-5554 ./gradlew connectedDebugAndroidTest -Pandroid.testInstrumentationRunnerArguments.class=com.example.notesapp.navigation.WebBookmarkAppShellVisualFlowTest#capturesAddBookmarkPageFromAppShell && adb -s emulator-5554 pull /sdcard/Download/web_bookmark_add_page.png "$FEATURE_DIR/visual_evidence/web_bookmark_add_page.png" && test -s "$FEATURE_DIR/visual_evidence/web_bookmark_add_page.png" |
