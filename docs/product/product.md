...
| web-bookmark | Web Bookmark in Notes | [docs/product/2026-09-11-web-bookmark/](2026-09-11-web-bookmark/) | In Progress | 2026-09-12 | US-1 (add flow) `passing`: 12/12 acceptance commands green. US-2 implementation and scoped verification are complete: 17/17 acceptance rows, real browser boundary, edit/delete journeys, export/compatibility tests, and five source-fed captures pass. The production actions sheet now matches its approved mockup and scores 0.9958 in the scoped comparison; final visual golden-baseline approval remains before lifecycle completion. |
<!-- HARNESS_TRACKER_END -->
...
| Internal note links and formatting contract protection (US-4) | ✅ Done | Full-screen NoteLinkPicker route with candidate search, folder subtitles, touch target compliance, self-exclusion, and link removal; preserved selected labels and collapsed-cursor target title insertion; primary-color underlined clickable note links; cascading deletion cleanup on target note removal and unresolved fallback; read-only protection of all 8 controls; verified visual flows. |
...
| Web bookmark management, browser boundary & export (US-2) | 🟡 In progress | Saved cards now expose Open and More actions; Edit preserves block identity and metadata, Delete is confirmation-gated with read-only protection, validated `ACTION_VIEW` launch and no-handler feedback are covered on the emulator, malformed/legacy content remains safe, and export/visual-flow tests pass. Five captures and reference-anchor evidence are recorded; golden-baseline promotion is pending. |
...
| Feature | Status | Notes |
|---|---|---|
| Export Formats | ✅ Done | Export notes to plain text, Markdown (.md), Markdown chart packages (.zip), and PDF formats under `Documents/NotesTakingApp/`; formula sources use `$source$` in Markdown and rendered formula text in PDF, while chart exports include relative PNG assets and localized table fallbacks. |
...
- **Web Bookmarks**: 🟡 Partially delivered — the metadata-enriched add flow is shipped, and US-2 card management, real browser boundary, export/compatibility handling, and runtime visual captures are implemented and verified. Final visual golden-baseline approval remains before the feature is complete.
