...
| Design: keyboard-visible picker layout | Full-height-above-IME picker keeps the focused search result visible while typing | US-3 | TC-US-3-VIS-004 | In scope |
...
3. **AC-US-3-03 Given** completed picker states, **When** production UI tests navigate to content, read-only, empty-search, and keyboard-visible search states, **Then** each state is asserted and its non-empty screenshot is captured for design comparison.
...
| TC-US-3-VIS-004 | AC-US-3-03, Design: keyboard-visible picker layout | Visual verification | `app/src/androidTest/java/com/example/notesapp/editor/EmojiPickerVisualFlowTest.kt#emojiPickerExpandsToAvailableHeightWhenKeyboardIsVisible` | Focus the production search field, type `launch`, and keep the IME visible while asserting the picker expands above it. | The sheet is taller than the IME-hidden two-fifths height, reaches the top of the available window, the filtered rocket result remains displayed, and the non-empty keyboard-state capture is saved for design comparison. | `env ANDROID_SERIAL=emulator-5554 ./gradlew connectedDebugAndroidTest -Pandroid.testInstrumentationRunnerArguments.class=com.example.notesapp.editor.EmojiPickerVisualFlowTest#emojiPickerExpandsToAvailableHeightWhenKeyboardIsVisible && adb -s emulator-5554 pull /sdcard/Download/notesapp_emoji_picker_keyboard_light.png "docs/product/2026-08-15-note-emoji/visual_evidence/emoji_picker_keyboard_light.png" && test -s "docs/product/2026-08-15-note-emoji/visual_evidence/emoji_picker_keyboard_light.png"` |
