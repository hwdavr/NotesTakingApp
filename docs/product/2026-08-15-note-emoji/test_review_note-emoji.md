...
| `env ANDROID_SERIAL=emulator-5554 ./gradlew connectedDebugAndroidTest -Pandroid.testInstrumentationRunnerArguments.class=com.example.notesapp.editor.EmojiPickerVisualFlowTest#emptySearchEmojiPickerLightTheme` plus pull/size check | 0 | Revised screenshot pulled to `visual_evidence/emoji_empty_search_light.png` (74696 bytes). |
...
| AC-US-3-03 | Content, read-only, empty-search, and keyboard-visible states are asserted/captured. | Four visual-flow states plus `#pickerSupportsRtlTraversalAndLargeFontScale`. | Production content and screenshot capture. | Tags/semantics, non-empty PNGs, filtered keyboard-visible geometry/results, RTL/1.5x traversal. | Visual class 5/5; screenshot pulls. | PASS | PASS unchanged |
...
| Edge: configuration/process recreation | Restore sheet state and cleanup. | `EmojiPickerLifecycleTest#pickerOmitsTitleAndHeaderCloseButton`, `#scrimTapDismissesSheetWithoutChangingDocument`, `#systemBackDismissesSheetBeforeLeavingEditor`, `#savedPresentationStateRestoresOpenSheetAfterRecreation`. | Header omission, scrim, Espresso back, `StateRestorationTester`. | Sheet dismissal/restoration and unchanged editor content. | Lifecycle class 4/4. | PASS | Fixed ✅ |
