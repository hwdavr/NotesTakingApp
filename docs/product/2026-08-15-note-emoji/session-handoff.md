...
- The production path inserts exact default and skin-tone Unicode through the shipped screen/ViewModel chain and records Recent. Catalog-error, empty-category, read-failure, lifecycle, missing-glyph, downstream Markdown/PDF, RTL/1.5x font-scale, two-fifths-height, header-omission, and expanded-catalog boundaries are covered.
...
- Visual evidence: `visual_evidence/emoji_picker_content_light.png` (138252 bytes), `emoji_read_only_light.png` (44621 bytes), `emoji_empty_search_light.png` (74696 bytes); design mockup v2 is `design/mockup_note_editor_emoji_picker_v2.png` (1103095 bytes).
...
- The latest layout revision is the user-approved final UI direction: two-fifths available height, no `Emoji` title, and no header cross button; the search field's query-clear action remains.
