...
| 13 | `./gradlew koverLog` | Coverage | PASS | Application line coverage 82.0053%, above the required 80%. |
...
| 18 | Full connected regression | Testing | PASS | `env ANDROID_SERIAL=emulator-5554 ./gradlew connectedDebugAndroidTest` completed `OK (168 tests)` with 0 skipped/failures. |
| 19 | Compose/localization/architecture scripts | Standards | PASS | All three custom rule checkers exited 0 with 0 violations. |
...
| 22 | Lifecycle tracker | Harness | PASS | `check-feature-lifecycle.sh` exited 0 after the tracker moved to `To be reviewed`. |
